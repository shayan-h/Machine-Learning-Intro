...
def my_dense(a_in, W, b):
    ...
    for j in range(units):               
        w = W[:,j]  # Selects all rows and jth column                                    
        z = np.dot(w, a_in) + b[j]         
        a_out[j] = g(z)               
    return(a_out)


# Builds a neural network using the my_dense() function
...
# Already trained weights from previous lab (normally these would be yielded through gradient descent)
...
def my_predict(X, W1, b1, W2, b2):
    ...
    p = np.zeros((m,1))  # A output y matrix, with one probability for each m (training example)
    for i in range(m):
